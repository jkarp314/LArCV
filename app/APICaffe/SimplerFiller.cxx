#ifndef __SIMPLERFILLER_CXX__
#define __SIMPLERFILLER_CXX__

#include "SimplerFiller.h"
#include "DataFormat/UtilFunc.h"
#include <random>
namespace larcv {

  static SimplerFillerProcessFactory __global_SimplerFillerProcessFactory__;

  SimplerFiller::SimplerFiller(const std::string name)
  ...
  void SimplerFiller::child_configure(const PSet& cfg)
  {
  ...
  void SimplerFiller::child_initialize()
  { _entry_data.clear(); }

  void SimplerFiller::child_batch_begin() {}

  void SimplerFiller::child_batch_end()   {}

  void SimplerFiller::child_finalize()    {}

  void SimplerFiller::set_dimension(const std::vector<larcv::Image2D>& image_v)
  {
    if(image_v.empty()) {
  ...
  void SimplerFiller::assert_dimension(const std::vector<larcv::Image2D>& image_v)
  {
  ...
  void SimplerFiller::fill_entry_data( const std::vector<larcv::Image2D>& image_v,
	...
	if(apply_smearing)
	  mult_factor = (float)(d(gen));
		...
		if(apply_smearing) val *= (_adc_gaus_pixelwise ? d(gen) : mult_factor);
                ...
                //val = ( val < 0. ? 0. : val);
                _entry_data[output_idx] = val; //( val > _max_adc_v[ch] ? max_adc : val);
